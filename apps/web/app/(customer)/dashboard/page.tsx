...
export default function CustomerDashboardPage() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-950 via-purple-950 to-indigo-950 p-8 border border-purple-500/30 shadow-2xl text-white">
        <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-purple-600/30 to-fuchsia-600/20 blur-[80px] pointer-events-none -z-10" />
        
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-purple-500/20 border border-purple-400/40 text-xs font-semibold text-purple-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white dark:bg-slate-900/90 p-6 rounded-2xl border border-slate-200 dark:border-purple-500/20 shadow-sm dark:shadow-xl flex items-center justify-between transition-all hover:border-purple-500/40">
          <div>
          ...
          <div className="w-12 h-12 rounded-xl bg-purple-50 dark:bg-purple-500/10 border border-purple-200 dark:border-purple-500/30 flex items-center justify-center text-purple-600 dark:text-purple-400">
      ...
      <div className="bg-white dark:bg-slate-900/90 p-6 rounded-3xl border border-slate-200 dark:border-purple-500/20 shadow-sm dark:shadow-2xl grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
        <div className="lg:col-span-2 relative h-64 md:h-80 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
          ...
          <div className="absolute bottom-4 left-4 right-4 flex flex-wrap items-center justify-between gap-2 text-xs font-mono text-purple-300 bg-slate-950/90 backdrop-blur-md p-3 rounded-xl border border-purple-500/30">
        ...
        <div className="space-y-4">
          <div className="text-xs font-extrabold uppercase tracking-widest text-purple-600 dark:text-purple-400">AI DAMAGE SURVEY</div>
          ...
          <div className="space-y-2 pt-2">
            <div className="flex justify-between items-center text-xs p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-purple-500/20">
              <span className="text-slate-700 dark:text-slate-300 font-medium">Front Bumper Scratch</span>
              <span className="text-purple-700 dark:text-purple-400 font-bold">$280.99</span>
            </div>
            <div className="flex justify-between items-center text-xs p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-purple-500/20">
              <span className="text-slate-700 dark:text-slate-300 font-medium">Left Front Door Dent</span>
              <span className="text-purple-700 dark:text-purple-400 font-bold">$465.00</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900/90 rounded-3xl border border-slate-200 dark:border-purple-500/20 overflow-hidden shadow-sm dark:shadow-2xl">
        <div className="p-6 border-b border-slate-200 dark:border-purple-500/10 flex justify-between items-center">
          <div>
          ...
          <span className="text-xs font-semibold px-3 py-1 rounded-full bg-purple-50 dark:bg-purple-500/10 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-500/30">
        ...
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 dark:bg-slate-950/80 text-slate-600 dark:text-slate-400 font-bold uppercase text-[10px] tracking-wider border-b border-slate-200 dark:border-purple-500/10">
                <tr>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-purple-500/10">
                {claims.map((c: any) => (
                  <tr key={c.id || c._id} className="hover:bg-slate-50 dark:hover:bg-purple-500/5 transition-colors">
                    ...
                    <td className="p-4 font-bold text-purple-700 dark:text-purple-300">₹{c.estimatedAmount?.toLocaleString()}</td>
                    <td className="p-4"><StatusBadge status={c.status} /></td>
                    <td className="p-4 text-right pr-6">
                      <Link
                        href={`/claims/${c.id || c._id}`}
                        className="inline-flex items-center gap-1 py-1.5 px-3.5 rounded-full bg-purple-50 dark:bg-purple-500/10 hover:bg-purple-100 dark:hover:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-500/30 font-semibold text-xs transition-colors"
                      >
                        Track Status
