    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900/90 p-6 rounded-3xl border border-slate-200 dark:border-purple-500/30 shadow-sm dark:shadow-xl">
        <div>
          <div className="flex items-center gap-2">
            ...
            {riskLevel === 'HIGH' && (
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300 border border-rose-300 dark:border-rose-500/40 flex items-center gap-1">
                <AlertTriangle className="w-3 h-3 text-rose-600 dark:text-rose-400" />
                HIGH RISK FILTER ACTIVE
              </span>
            )}
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Filter, sort, and inspect submitted insurance claim files</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-purple-500/30 rounded-3xl shadow-sm dark:shadow-xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-xs text-slate-500 dark:text-slate-400">No claims match the specified filter criteria.</div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-purple-500/10">
                {claims.map((c: any) => (
                  <tr key={c.id || c._id} className="hover:bg-slate-50 dark:hover:bg-purple-600/10 transition-colors">
                    ...
                    <td className="p-4 font-extrabold text-emerald-600 dark:text-emerald-400">₹{c.estimatedAmount?.toLocaleString()}</td>
